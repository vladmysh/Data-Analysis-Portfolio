/*
 ...
 Данный запрос строит помесячный график платежей по кредиту от даты предоставления по дату окончания займа, исходя из данных, которые определяются при выдаче кредита.
...
/*
 Поиск второй ставки кредита. В первую очередь берется ставка со страховкой, при ее отсутсвии - без страховки.
 Если нет обоих вариантов ставок, то у займа отсутствует перемена условий и остается первая ставка.	
*/	
SET @rate_2 = CASE WHEN (SELECT [Value] FROM #rates_info_table WHERE [nameKod] = 'rate_wi') IS NOT NULL
	            AND (SELECT CAST([Value] AS decimal(7,4)) FROM #rates_info_table WHERE [nameKod] = 'rate_wi') != 0
...
/* Расчет количества месяцев действия 1-й ставки */
SET @months_to_end_r1 = COALESCE(IIF(DAY((SELECT [dOpen] FROM #credit_info_table)) >= DAY((SELECT [Value] FROM #rates_info_table WHERE [nameKod] = 'date_change')),
...
/*
 При наличии даты, когда будет первый платеж по новой ставке, исчисляем дату первого платежа по кредиту.
 При ее отсутствии принимаем, что первый платеж будет через месяц после предоставления займа.
*/
SET @first_pay_date = CASE WHEN (SELECT [Value] FROM #rates_info_table WHERE [nameKod] = 'date_change') IS NOT NULL
			   THEN (SELECT DATEADD(month, -1 * @months_to_end_r1 + 1, [Value]) FROM #rates_info_table WHERE [nameKod] = 'date_change')
			   ELSE (SELECT DATEADD(month, 1, [dOpen]) FROM #credit_info_table) END
...
/*
 Рассчитываем платеж по процентам.
 Если следующий платеж в январе, то необходимо учесть смену года на високосный. Часть декабрьского платежа будет отличаться от части январского.
*/
SET @interest_payment = CASE WHEN MONTH((SELECT [Дата платежа] FROM #main_table)) = 12
			     THEN

		        (SELECT IIF(@count <= @months_to_end_r1, @rate_1, @rate_2)
			        / CASE WHEN YEAR([Дата платежа]) % 4 = 0 AND YEAR([Дата платежа]) % 100 != 0 THEN 366
				       WHEN YEAR([Дата платежа]) % 400 = 0 THEN 366
				       ELSE 365 END -- алгоритм определения количества дней в году (проверка на високосный год)
...
SET @principal_payment = (SELECT IIF(@count <= @months_to_end_r1, @principal_payment_r1,
				   IIF(@count = @total_months OR (SELECT [ОД на начало месяца] FROM #main_table) < @principal_payment,
				       (SELECT [ОД на начало месяца] FROM #main_table),
				       IIF(@interest_payment > @annuity, 0, @annuity - @interest_payment))))

/* Формула вычисления платежа по страхованию залогового имущества */
SET @insurance_amount = IIF(MONTH(@next_month_pay_date) = MONTH((SELECT [dOpen] FROM #credit_info_table))
			    AND YEAR(@next_month_pay_date) != YEAR((SELECT [dOpen] FROM #credit_info_table)),
			       (SELECT ([ОД на начало месяца] - @principal_payment) * 1.3 * 0.007 FROM #main_table), 0)
...
SELECT CAST([Дата платежа] AS nvarchar(10)) AS [Дата платежа],
       CAST([Сумма платежа] AS money) AS [Сумма платежа], -- тип данных "money" удобен для переноса полученного результата запроса в Excel-таблицу
       CAST([Погашение основной суммы кредита] AS money) AS [Погашение основной суммы кредита], -- поскольку в этом числовом типе данных знак-разделитель ","
